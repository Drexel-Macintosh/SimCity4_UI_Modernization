...
| ~~3~~ | ~~**Zots** (no-power/water/job/car discs) — no census row at all~~ **CLOSED 2026-08-23** — decoded offline end-to-end (5-lens workflow, every VA independently re-derived, 0 refuted): the four ids are HARD-CODED (7 push-imm32 `.text` sites), spawned as **city props** by four updaters of the row-13 drawer-host class + the landfill simulator's NoCar outlier `0x6BB3B0`, via `AddZot 0x6C98C0` (kind priority 4=NoPower > 5=NoWater > 6=NoCar > 7=NoWork, attr `0xC999C45E`, prop stamp `0x48E95539`) → `cSC4PropManager::AddLotProp` vt+0x68 (`0x4A2670`, ⚠ header says +0x64 — overload pair swapped) at building-top +3.0f. Sized by **S3D vertex metres** (80 models, `I = base\|zoom<<8\|rot<<4`); OccupantSize `{12,12,2}` = simulator footprint only; **world-anchored, zero px→world callers → correct at every tier by construction, click-transparent on pick `0x4B8880`**. Exactly four zots ship. Census row 23 + full chain: `tools/research/overlays/row-23-zots.md`. Grade PARTIAL until the one-screenshot zoom-pair probe runs (TastyZots cheat `0xAA7A2746`). | | | | |
...
| ~~3~~ | ~~Sweep the exe for the four Zot instance ids as imm32...~~ **DONE 2026-08-23** — the sweep had already run (`exe-instance-sweep.txt`, control PASS): 7 hits, all engine-pushed. Creator chain fully decoded and adversarially verified; see §B.1 item 3 and `tools/research/overlays/row-23-zots.md`. The one remaining live step is the §5 zoom-pair screenshot probe. |
...
| **8** | **~~`src/UiSpike.cpp.before-iconprobe-2026-08-14` (732 KB, Aug 14) sits in `src/`.~~ RESOLVED 2026-08-23 — deleted in the repo-hygiene sweep.** It duplicated every symbol in both of my greps this session. In a published tree it doubled every hit a modder got on `kNeverScaleIds`, `ScalePanelRoot`, the BMPX block and the NineSlice comment — with the *old* values. | Yes |
