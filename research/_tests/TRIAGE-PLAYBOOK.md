...
| `START-HERE.md` §5 | the 22 standing rules. This file is those rules re-indexed by symptom. |
...
> **#162, 2026-08-15.** Six fixes missed in a row because every single one hunted
...
after the sixth.**
⚠ `-Mode Stock` leaves the mod DISABLED. `-Mode Ours` to restore — and
`START-HERE.md` §6 warns the tree may already be sitting in stock mode.
...
:: STEP 1 — is the tree in a known-good state? (see §5 for what is red today)
...
bleed), #162 (two phantom hairlines — **still open, mechanism found, kill test
pending**).
...
Its live output right now (~~run 2026-08-15, this session~~ **re-run 2026-08-16.
The numbers below still reproduce exactly, but the #170 rewrite split the staged
half into RUNTIME-SCALED and PRE-SCALED DATA, so the gate now prints seven
summary lines per tier — four from `run()` (`gate_btn_undercover.py:273-280`)
and three from `run_static()` (`:409-412`). The 2026-08-15 transcript predates
the pre-scaled line entirely: before that rewrite those nodes "fell out at
`continue` and were never counted" (`REGRESSION.md:10290-10295`)**):
...
both integer tiers**. That is a live open lead, not background noise.

⭐ **AND IT IS NOT THE ONLY LINE THE RULE CONDEMNS (added 2026-08-16).** Quoting
one of the seven lines is how a second population stayed invisible. The gate
reports **three** 1.5x-only residual populations — 34 runtime (`:276-277`), 347
static (`:411-412`), and **132 pre-scaled** (`:278-280`) — each zero at 2x and
3x. The 132 are the ones with their own id: the window is right (`ship == want`
for all 132, measured) and the **SHEET** is over-snapped by `ScaleDim`'s
`CellUnit` — Zoom Out is 21px at 1x and `R(21*1.5)=32`, but the 84px four-state
sheet divides by both 3 and 4, snaps on LCM 12, lands at 132, cell **33**. That
is **#171**, open (`REGRESSION.md:10300-10308`). All three share one cause
family, and the cure for all three is an ART-dimension change that is reverted
and scoped game-wide (#148/#156) — **reported, not failed**. Read every line the
gate prints, not the one this block quotes.
...
| **the same cure missing from ~~the other path~~ ANOTHER path** — *corrected 2026-08-16: there are **THREE** paths that must carry this rule, not two; `build_selective_safe.py:714-721` names all three* | the same control is 83px at runtime and 82px in a static dat | `build_dialog_static.py::leaf_art_sized()` (`tools\dialog-static\build_dialog_static.py:748`). Static dialogs are *deliberately* excluded from the sweep, so nothing repaired them | #155 v2.98.0 |
...
| **tiled sheet snapped when it has no cell divide** | `blttype=tiled`; art 111x528 vs window 111x527 | ~~`find_tiled.py` → `no-snap.txt`~~ ⛔ **WRONG GENERATOR — `find_no_snap.py` → `no-snap.txt` (121 sheets) → `Upscale2x --no-snap`.** Corrected 2026-08-16: `find_tiled.py` defaults to `--out tiled.txt` (`find_tiled.py:100`) and emits only the **10** PROVEN-tiled TGIs; `no-snap.txt:2` names `find_no_snap.py` as its generator, and `Rebuild-Corpus.ps1:72` is what actually feeds `no-snap.txt` to `--no-snap`. `tiled.txt` is a strict SUBSET of `no-snap.txt` (0 entries outside it), so regenerating the shipped list from `find_tiled.py` drops **exactly 111** sheets — every one admitted by #162's "bound 1:1 to a window of exactly its 1x size" rule — because `tiled.txt:1` is scoped `#160` alone while `no-snap.txt:1` is `#160 + #162`. Nothing fails: the file still exists, `Rebuild-Corpus.ps1:75` only tests `Test-Path`, and every downstream gate stays PASS — the §M-item-2 shape. The role table ten lines below had it right. | #160 |
...
⭐ **THE OFFSET-PARITY LAW (rule 17) NAMES THE FAILING AXIS BEFORE YOU LOOK.**
...
STARTED** (rule 19). CAM's Village Hall info screen rendered at 1x under 1.5x
...
⭐ **THAT IS A BISECTION BOUNDARY, NOT AN OPINION** (rule 15). Four defects were
...
2. **Is a build INPUT stale?** `tools\upscale\preview\`, `preview-15x\`,
   `preview-3x\` ~~are generated by a **hand-typed** command and read for
   weeks. Nothing in the repo regenerates them.~~ **CORRECTED 2026-08-16
   (#170):** those three trees now have a repo regenerator —
   `tools\upscale\Rebuild-Corpus.ps1:94` maps `1.5→preview-15x`, `2→preview`,
   `3→preview-3x` and `:96-109` writes them. Everything else in this list was
   NOT re-checked and is still assumed hand-made — including the three
   `.txt` lists themselves, which `Rebuild-Corpus.ps1:76-78` confirms are
   still produced separately by `find_cell_strips.py` and friends. Same shape:
   `cell-strips.txt` / `nine-slice.txt` / `no-snap.txt`, `refmap-<tag>.csv`
   (dialog-static reads the one selective-safe wrote — **hard ordering
   dependency**), `nam-up-*`, `uncovered-up-*`, `extracted-plugins\`.
   ~~⚠ **ARMED RIGHT NOW:** the upscale step documented in
   `tools\packages\PACKAGES.md` (the `Upscale2x.exe … --factor … --normalize-names`
   block) carries **neither `--cell-strips` nor `--nine-slice` nor `--no-snap`**.
   Anyone who follows the doc silently un-ships #156, #157 and #160 while every
   downstream gate still reports PASS.~~
   ✅ **DISARMED 2026-08-16 (#170).** That block is gone: live
   `tools\packages\PACKAGES.md` now contains **zero** `Upscale2x.exe`
   invocations. Step 1 (`PACKAGES.md:333-344`) runs
   `upscale\Rebuild-Corpus.ps1`, the single source for the command
   (`Rebuild-Corpus.ps1:2-4`). It appends all three derived lists itself
   (map `:69-73`, append `:101-102`) and refuses to run when a list is
   **missing** (`:75-80`, no override) or **empty** (`:82-86` — overridable,
   but only by explicitly passing `-AllowEmptyLists`, `:48`). Use `-DryRun`
   (`:104-107`) to print the exact command line without spending a corpus
   rebuild; that is the cheap answer to "did we pass the lists?".
   ⛔ **Only the doc pointer was fixed — the hazard is permanent.** A
   hand-typed `Upscale2x.exe` line still un-ships #156/#157/#160 at exit 0
   with every gate green, because each gate measures the new tree against
   itself. **Never hand-type it, never copy it into a doc, run the script.**
   The flagless command still exists verbatim in
   `_working-backup\PRE-OVERNIGHT-2026-08-15-2145\tools\packages\PACKAGES.md:335-336`
   and `_tests\HARDENING-PROPOSALS.md` (lines 845-846 as of 2026-08-16 — that
   file moves, so find it by CONTENT: `grep -c "Upscale2x.exe dbpf"`, its own
   rule at `:19`) — do not copy from either.
...
A correct fix was reverted on that (#153, rule 18). **Do not machine-generate
...
| question | gate | integer control | status, run 2026-08-15 |
|---|---|---|---|
| does the shipped art cover the sweep-scaled window? | `python tools\uimap\emu\gate_art_vs_window.py` | ✅ HARD — `NEW at f=2 (must be 0)` | **GREEN.** 1038 short at f=1 (stock baseline, subtracted), **0 new at f=2, 0 new at f=1.5** |
| does a state-strip button's window equal its art cell? | `python tools\uimap\emu\gate_btn_undercover.py` | ✅ 15x/2x/3x; integer no-op by construction | **PASS with a live lead** — static residual 15x `{(0,2):347,(0,6):3,(0,1):1,(1,0):1}`, **none at 2x, none at 3x** |
| do abutting siblings SEPARATE after parent-frame rounding? | `python tools\uimap\emu\gate_abut_1_5x.py` | ✅ HARD — FATALs "MODEL IS WRONG" | **GREEN, 0/0/0/0** — the abut hypothesis for #162 is refuted offline |
| is the minimap blit size legal at every tier? | `python tools\uimap\emu\gate_minimap_snap.py` | ✅ FACTORS=(1.5,2,3) | **GREEN** — `1.5→96 strip, 2→128 skip, 3→192 strip` |
| are the NAM ItemIcon overrides present, exact, and winning? | `python tools\uimap\emu\gate_namicons.py` | ✅ TIERS 15x/2x/3x + 5 negative controls | ⛔ **RED** — 392 orphans + 1 losing icon (`0xD6482A2C ← DBSSY_Notre_Dame_de_Paris_2025`). Known as task #152, red on a pre-#149 assumption |
| do patch families overlap or split ownership of a constant? | `python tools\uimap\emu\gate_patch_families_combined.py` | ➖ exe-level | ⛔ **RED — 5 UNREGISTERED TABLES**, all the **#159 cost-box family**: `kCostBoxHeightSite`, `kCostBoxWidthSite`, `kCostOriginBack`, `kCostOriginSite`, `kCostOriginStock`. **The gate is therefore silently blind to a whole patch family** — the exact gap #106 was built to close |
...
| third-party `blttype=normal` blits — all three numbers | `gate_tp_bmp_fit.py` | ⚠ control is **1x vs tier**, not integer-tier. **Has been wrong twice — read its header** | negative control = a script extracted back out of a deployed v2.97.0 dat → 48 findings |
| the graph-legend byte patch | `gate_graphlegend_leftanchor.py` | ✅ f=1 reduction | **THE BYTE GATE** — 127 checks. `--emit` prints the exact hex `CodePatches.cpp` must write. ⭐ **hand-encoded bytes get diffed against `--emit`, always** (law 50) |
| the advice-row / ordinance / intro-video / graphs-band patches | `gate_advice_rowx.py`, `gate_ordinance_namex.py`, `gate_introvid.py`, `gate_graphs_banddock.py` | ✅ all carry f=1 reduction or 6 mandatory negative controls | green |
| sub-flyout native offset per tier | `python tools\flyout-sim\gate_subnative.py` | ✅ 3 tiers | green |
...
| **runtime sweep** (`UiSpike`) | ~~any window in the tree~~ **any window its walk actually reaches — the ROOT of a `kDataScaledSubtreeIds` panel IS scaled and anchored, but the walk STOPS there**, idempotent via `scaleMap` (keyed on POINTER) | cannot reach content painted inside a buffer, or anything re-laid after the sweep — **and ⛔ NOTHING BELOW a `kDataScaledSubtreeIds` root: `ScalePanelRoot` scales/moves the root, then RETURNS before its child loop (`src\UiSpike.cpp:14557`; loop at `:14566`), so no descendant is walked, at any tier — the return has no factor test. TEN ids, `src\UiSpike.cpp:5373-5486` — ⚠ READ THE ARRAY, do not trust a hand-list: advisor strip `0x6A15C767`, three Graphs roots, two U-Drive-It roots, AND four budget/taxes roots (`0xAA3AC002`, `0xCA4C332D`, `0xAA3AC001`, `0xAA3AC000`, `:5466-5469`). Below the root, geometry ships pre-scaled in the DATA (`double_subtree_areas`) and the number in the `.UI` is the number on screen — so a 1.5x defect on the advisor row is a BUILDER defect, never `ScaleSubtree`, which provably never runs there (#170). The ROOT's own rect is the opposite: it is the sweep's OUTPUT, not the data's (`:5444` logs `panel 0xAA3AC002 (158,40 500x464) -> (316,80 1000x928)` across 14 capture runs; `:5435-5436` "the sweep scales + anchors each root and never descends"). Scope, stated no stronger: `ScaleSubtree` has NO such gate (`IsDataScaledSubtreeId` has one call site, `:14557`) — the guarantee holds because these roots are only entered through `ScalePanelRoot`. Corrected 2026-08-16, #170.** | none |
...
`src\UiSpike.cpp` and `tools\upscale\Upscale2x.cs`, and `START-HERE.md` §5.
Gate statuses in §5 were measured by running them, 2026-08-15.
When you close a defect, add its branch row here in the same session — a failure
list without mechanisms just gets retried in a different order* (law 23).
