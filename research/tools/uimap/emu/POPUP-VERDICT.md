...
3. The fix is a three-constant `round(stock × f)` patch that reduces to stock
   ...
   `f = 2` (§4).
4. **§5 (added after the coordinator's screenshot report) closes the other
   half.** The line breaks are computed by `sub_9BF3E0` against
   `[this+0x160] = GetW() − 2×gutter(5)` — **recomputed and re-broken on every
   `SetArea`** (override `0x009BFCA5`), so the effective wrap width today is
   `750 − 10 = 740`, not 1000. It does not wrap because the **word-wrap flag
   `0x0002` in `[this+0x128]` is off** (ctor default 0; `sub_779660` never sets
   it), leaving the `'\n'`-only regime — which is exactly why one description
   breaks early and another clips mid-word. **Cure: one
   `cIGZWinText::SetWinTextFlag(2, true)` on `0x0ABCE001`. No constant, no
   string work.**
...
|---|---|---|---|---|
| BODY 2x, x/y **unpatched** (15,25) | popup 840x125 | **795x75** | 795x75 (City Lottery dump) | ✅ |
| BODY 2x, x/y **patched** (30,50) | popup 840x125 | **750x25** | 750x25 (Smoke Detector dump) | ✅ |
| TITLE 2x, align 0 | text extent 303x37 | **697x37** | 697x37 | ✅ |
...
|---|---|---|---|---|
| `0x0078B99F` | `push 0x7d` (imm8 @ `0x0078B9A0`) | **125** | popup **height** (`SetSize` on both `0x0423278D` and content `0x0423278F`) | ❌ **NO** |
| `0x0078B9A1` | `sub ebx,0x3c` (imm8 @ `0x0078B9A3`) | **60** | popup width = `dialogW − 60` | ❌ **NO** |
| `0x0078B9D7` | `push 0x1e` (imm8 @ `0x0078B9D8`) | **30** | popup **x** | ❌ **NO** |
| `0x0078B9C3` | `add eax,-0x7d` | **125** | the y clamp (`y ≤ dialogH − 125`) | ❌ **NO** |
| `0x0078BA2B` / `0x0078BA29` | `push 0xa` / `push 5` | 10 / 5 | TITLE x / y | ✅ v2.27.0 |
| `0x0078BA77` / `0x0078BA75` | `push 0xf` / `push 0x19` | 15 / 25 | BODY x / y | ✅ v2.27.0 |
...
| body vs 2×stock | — | **+60 wide, −125 tall** | 690 = 2×345 ✅, 150 = 2×75 ✅ |
...
(`STOCK-REFERENCE.md`: *output = stock scaled, judged by geometry*). It does
...
Added 2026-07-30 after the coordinator reported live 2x screenshots in which
one description breaks early with space left in the box and another runs to
the box edge and is cut mid-word. **Both behaviours are one mechanism.**
...
constants**: 335 at 1x, 680 after the §4 geometry fix at 2x, 1025 at 3x. It is
idempotent (setting a set flag is a no-op), it survives any later resize by
design, and it needs **none** of the `CalculateWordsToFitInWidth` string work
in the in-flight v2.28.0 block.
...
## 5A. OLDER NOTE — kept for the record (superseded by §5)

What is **proven**:

* The only layout call is `FitWindowToText(false,false)` at step 5, i.e.
  **before** the `1000` SetArea and **before** the fill SetArea. Whatever width
  the layout used, it was the window's width *as the factory created it* —
  never 1000, never 750, never 840. The "wrap at the unscaled 1000" hypothesis
  is refuted on **ordering**, which is stronger than a value argument.
* Nothing in `sub_779660` sets a word-wrap flag. It only *clears* window flags
  `0x800` (Sortable) and `0x8000` (AcceptFocus) — `cIGZWin::SetFlag`, not
  `cIGZWinText::SetWinTextFlag`.
* §POPUP P4 question 3's candidate flags are a **false lead**: the sites
  `0x777299`, `0x777424`, `0x777479` call `[vt+0x18](flag)` then
  `[vt+0x1C](flag)` with **one** argument. `cIGZWinText::SetWinTextFlag`
  takes **two** (`flag, value`) — confirmed by a genuine two-argument call
  `push 0; push 2; call [eax+0x1C]` at `0x009C7DA9`. Those `0x8001` / `0x8010`
  sites belong to a different class and mean nothing here. `0x0077899B` is
  likewise a one-argument getter on a non-text object.

**Superseded by §5**: the guess that the layout is frozen at creation width was
wrong — the `SetArea` override re-derives it, so the *last* `SetArea` decides.
The conclusion that follows from §5 instead: **stock does not wrap this text
either** (flag `0x0002` is off at every tier), so the 1x capture will show the
same `\n`-only breaks plus clipping, just at 345 px. Take it anyway — it is 3
minutes (`BUDGET-DETAIL-ANATOMY.md` §POPUP P5) and it is the only thing that
distinguishes "we match stock" from "we broke it".
...
| `cIGZFont` vtable (header confirmed by `+0x8C = GetLineHeight`) | `+0xA4 CalculateWordsToFitInWidth`, `+0xB4/+0xB8 CalculateTextArea`, `+0xC0 GetStringWidth` — the manual-wrap route (§POPUP P4 question 4). **§5.5 makes it unnecessary.** |

**Note for the v2.28.0 work in `src\UiSpike.cpp`** (not edited by this pass):
its reasoning block states the body window "already self-scales — measured 750
wide inside the 840 box, which is right", and that the wrap happens against the
unscaled 1000. Both are refuted: 750 is **60 px too wide** (2×stock is 690) and
25 px tall against a 2×stock of 150, and the 1000 is applied *after* the layout
and then overwritten. Per §5.5 the manual `CalculateWordsToFitInWidth` +
newline-injection block can be **deleted entirely** — one
`SetWinTextFlag(2, true)` makes the engine do it, correctly, at every tier, and
re-do it on every resize.
