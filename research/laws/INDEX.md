...
- [SELECTOR FREEZE: the instrument names it, the warm thread cures it](feedback-selector-freeze-named-by-instrument.md) — two inference fixes missed; ONE in-memory timing pass named `EnumDisplaySettingsW` 3.3s in one launch (a once-per-session cost scheduled at the first click); cure = warm the enumeration at DLL load on a background thread with a tri-state handshake; the instrument STAYED in the build
- [SELECTOR: derive, diff-apply, commit-at-close](feedback-state-machine-derive-diff-commit.md) — an accreted per-tick megafunction breeds two-sources-of-truth defects; cure = one state struct, one PURE derive (spec written FIRST as a python gate), rebuild a combo only when derived rows differ on a selection-change tick, commit only changed keys at close; the player's pick is a REQUEST — effective = request-if-usable-else-Auto, so bounce/un-bounce need no state
- [LIVENESS GUARD: pass the pointer found THIS pass](feedback-liveness-guard-stored-window-pointers.md) — with a city loaded the game DESTROYS Graphic Options on Accept (main menu only hides it); a stored dialog pointer re-read at close faulted — crash, NO exception report, no SELCLOSE line; null = destroyed: skip the re-read, zero filters WITHOUT RemoveMessageFilter on dead windows. Also: geometry from CLIP ARITHMETIC (a BMP area is l,t,r,b; children clip to the local rect), never screenshots; the combo drop-list paints the STOCK list colour regardless of .UI flags — join it, don't fight it
