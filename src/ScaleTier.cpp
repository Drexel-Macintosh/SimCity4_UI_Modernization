...
namespace
{
	...
	// ==========================================================================
	// THE LIVE FONT MUST BE CALLED FontStyle.ini - SO MAKE IT SAY WHOSE IT IS.
	//
	// A player asked for the file we drop into the game's own Plugins folder to
	// use a DIFFERENT name, so a copy stranded by a crash could never be
	// confused with their own. The right instinct, and measurably impossible:
	// the executable holds exactly ONE "FontStyle.ini" string (0x00A86DC8),
	// probed at TWO FIXED PATHS built by vtable getters (+0xDC, +0xD4) at
	// 0x0044DC85, with no recursive search and no subfolder probe. Rename it
	// and the game simply never reads it - the enlarged font silently stops
	// working. The name is not ours to choose.
	//
	// What we CAN fix is the thing the rename was actually for: a stranded copy
	// being indistinguishable from the player's own. The format is a text INI
	// whose every header line already begins with ';', so a comment line is
	// inert to the parser and visible to a human the moment they open it.
	//
	// ⭐ AND IT MAKES OUR OWN DELETE TEST BETTER. Ownership used to be decided
	// by byte-identity against a shipped tier source, which silently stops
	// recognising anything the moment a tier source is regenerated. A marker is
	// self-describing and survives that.
	static const wchar_t* const kFontMarkerW =
		L"; SC4UIScale: this file was generated by the SimCity 4 UI "
		L"Modernization mod.\r\n"
		L"; It is SAFE TO DELETE. If the mod is gone and this file is still "
		L"here, delete it -\r\n"
		L"; the game falls back to its own built-in font table.\r\n";
	static const char* const kFontMarkerA =
		"; SC4UIScale: this file was generated by the SimCity 4 UI "
		"Modernization mod.\r\n"
		"; It is SAFE TO DELETE. If the mod is gone and this file is still "
		"here, delete it -\r\n"
		"; the game falls back to its own built-in font table.\r\n";

	// Does this file carry our header? Read-only, cheap, and NEVER throws on a
	// short or unreadable file - a probe that cannot read must answer "not
	// ours" so we never delete something we failed to inspect.
	bool HasOurFontMarker(const wchar_t* path)
	{
		...
		buf[got] = '\0';
		return strstr(buf, "SC4UIScale:") != nullptr;
	}

	// Copy a tier source to `dst`, stamping our header on the front.
	// Falls back to a plain copy if anything goes wrong: an unstamped font that
	// works beats a stamped one that does not.
	bool CopyFontStamped(const wchar_t* src, const wchar_t* dst)
	{
		HANDLE hs = CreateFileW(src, GENERIC_READ, FILE_SHARE_READ, nullptr,
			OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr);
		if (hs == INVALID_HANDLE_VALUE) { return CopyFileW(src, dst, FALSE) != 0; }
		HANDLE hd = CreateFileW(dst, GENERIC_WRITE, 0, nullptr,
		...
		DWORD wrote = 0;
		bool good = WriteFile(hd, kFontMarkerA,
			static_cast<DWORD>(strlen(kFontMarkerA)), &wrote, nullptr) != 0;
		char chunk[8192];
		DWORD got = 0;
		while (good && ReadFile(hs, chunk, sizeof(chunk), &got, nullptr) && got)
		{
			good = WriteFile(hd, chunk, got, &wrote, nullptr) != 0 && wrote == got;
		}
		CloseHandle(hs);
		CloseHandle(hd);
		if (!good)
		...
		const wchar_t* activeTag /* nullptr = stock */)
	{
		...
		if (CopyFontStamped(src, live))
		{
			Logger::Get().WriteLine(
				LogLevel::Info,
				"ScaleTier: FontStyle%ls.ini -> %ls (stamped with our header, "
				"so a copy stranded by a crash names itself).", activeTag, live);
		}
		else
		{
