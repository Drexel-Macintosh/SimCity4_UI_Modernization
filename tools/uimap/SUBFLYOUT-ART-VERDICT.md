...
The live sizes recorded in `tools\research\MAYOR-MODE.md` are these values
doubled by the generic sweep. `src\UiSpike.cpp:2544` states the same pair
independently — *"SIZE was already correct via the generic sweep (129x241 ->
258x482)"* — matching 129 and 241 as derived from the immediates.
...
Container heights 482/384/874/776/580 come from `tools\research\MAYOR-MODE.md`,
and 482/286/206 from live observation of the same family. Strip heights
284/382/578/774 come from the archived-log list quoted in
`src\UiSpike.cpp:5727`. **Zero misses, zero fudge factors.**
...
that breaks every arithmetic progression fitted to the data (the other seven
are `90 mod 98`; 206 is `10 mod 98`), and the formula produces it exactly, from
a constant — `0x35` at VA `0x007EB163` — that no art asset supplies.

---

## 2. THE EVIDENCE, IN CODE

### 2.1 The builder is `sub_7EAEB0`

`sub_7EAC70` ends at `0x007EAEAD` (`ret`, then `int3 int3`). The function that
creates the container is a separate `__thiscall` at **`0x007EAEB0`**
(`ret 0x18` = 6 stack args), ending `0x007EB2E9`. Consequences:

* the `124` at `0x007EAD4B` is in the *other* function and is **not on this
  path**;
...
The argument→field mapping comes from the **stores inside `vf10`**, not from
push order.

### 2.3 `vf10` = `0x0079AC60` — where the art enters, and where it stops

Container object primary vtable = **`0x00AB6D04`** (written by ctor
`0x0079AFF0`; the `cIGZWin` sub-object at `+4` gets `0x00AB6AA8`).
`vf10 = [0xAB6D04+0x10] = 0x0079AC60`, `ret 0x20` = 8 args. It stores:
...
`lea ecx,[edi+edi]; sub eax,ecx` → `[+0xec] = artDim − 50`. Against the 292-wide
atlas of §3.1 that is `292 − 50 = 242`: the stretchable middle of a 9-slice
whose two end caps are the 25 in `[+0xe8]`. The field parameterises the blit
...
0x0079ADC7  call [edx+0xdc]  on [esi+4]           ; SetArea4 (+0xDC, tools\research\SC4-UI-ENGINE.md:145)
```
...
### 2.5 Cross-check: the runtime already patches these constants
...
**= the 53 pushed at `0x007EB169`**. `tools\research\MAYOR-MODE.md` measures
exactly this ("only right half clickable… claims only the rightmost
`[this+0xe0]` px") and cures it with **`ClaimScale=2`** — a runtime *doubling
of a code constant*. Likewise `gStripFieldScale=2` doubles the 44 cell. Two
runtime patches land on these very constants, which is only possible if they
are code, not art.
...
A dimension inventory (IHDR/JFIF/BMP/FSH headers) covers all 2,280
type-`0x856DDBAC` entries of `SimCity_1.dat`, the only archive with UI art
(`tools\dbpf\NOTES.md`).

### 3.1 What the builder actually loads

`0x007EB0C0–0x007EB0D4` fetches `{0x856DDBAC, 0x46A006B0, arg5}`. `arg5` is a
caller argument; all **seven** call sites of `sub_7EAEB0` push a literal:
...
**This is the first falsification, and it is decisive.** The art instance **does**
differ per menu — and its size **never** does. Seven different instances, one
size, seven different window heights from 103 to 437 stock. No art-driven
mechanism produces that.
...
**The 129-wide art.** There *are* 129-wide images: 24 of them (instances
`0x14416220`…`0x1441622B` × both mirror groups), and every one is exactly
**129 × 129** — square. They are excluded because (a) none appears as an imm32
anywhere in the exe, (b) all are `.UI`-referenced (`refmap.csv` rows 7/8/243–245…),
and (c) a single square asset cannot produce eight different heights against
one fixed width. The 129 in `W` is a *sum of three immediates*, `80 − 4 + 53`,
computed at `0x0079AD0A–0x0079AD23`; that it also happens to be an art size
elsewhere in the game is a collision, not a source. Recorded because a search
for 129 finds these first.

**The one 258-wide image is a red herring and is recorded as such.** Of 2,280
entries exactly one is 258 wide: `T-856ddbac G-46a006b0 I-14416322`,
...
(`T-00000000_G-96a006b0_I-ca539340.ui`, 3 refs); (c) 258 is the *doubled*
value — stock is 129 — so matching it would be matching the wrong number
anyway. A fresh search of the art finds that image first; this paragraph exists
...
`EDD` carry no `.UI` reference, so they are absent from `refmap.csv` and the
reference-driven staging pass can never reach them — the same blind spot that
`CODE_BOUND_TGIS` in `tools\selective-safe\build_selective_safe.py` exists to
patch.

The equivalent experiment on the strip's contents falsifies the art hypothesis
directly:

* the item icons are staged at **352 × 88** — 4 states × 88, doubled from stock
  `176 × 44`, across 320 icons. Shipped.
* The strip cell nevertheless stays **44**, because `provider->vf30(44,44,5)`
  hardcodes it.
* The cure is `gStripFieldScale=2` — a **runtime constant patch**.

Art at 2x moves the geometry zero pixels. Two runtime constant patches move it.
Combined with §3.1 (constant art, varying height) the art hypothesis is
...
## 5. WHY THE CONTAINER FLASHES, AND THE CONSTANTS THAT CURE IT

The container is **born** at `129 × 241` and painted there. The generic sweep
runs 4×/s and resizes it to `258 × 482`. That is the 1x-for-a-moment,
snaps-to-2x the player sees — a textbook reactive-sweep flash, whose cure is
...
### The constant table, with the value at each tier
...
**exactly** the sizes the sweep produces after the fact. The constant patch
reproduces that end state at construction.

Constraints on the patch:

* **`0x007EB165` (80) overflows `imm8` at f ≥ 1.6** (signed cap 127). f=1.5 is
  a pure byte-patch; f=2 and f=3 need `6a 50` → `68 imm32` (2 bytes → 5, so a
  cave or an in-place re-encode of the whole push block), or a runtime pin on
  that one site.
...
* The patch footprint is narrow: `sub_7EAEB0` is the *only* creator of
  ...
  though, has a **second** push site at `0x007E5EB9`, outside this builder —
  the strip id is not exclusive to `sub_7EAEB0`.
