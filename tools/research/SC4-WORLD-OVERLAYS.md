...
| 10 | building/lot selection glow (query & demolish hover) | **Occupant HIGHLIGHT FLAG** — `cISC4Occupant::SetHighlight(mode,sendNow)` at vt+0x44 (`cISC4Occupant.h:57-58`); the renderer tints the occupant's OWN model, so there is no separate visual. Query tool (clsid `0xC7AF928E`) sets mode **7** @`0x4CBF9D` (saves old at `[this+0x40]`, restores `0x4CBF65`); Demolish (`0x46DDB5F1`) sets **5** @`0x4B99F6`; mayor-default control sets 5 @`0x4DB34A`, clears @`0x4DAD7E` and spawns one `local_tile_outline` effect @`0x4DA77A`. Change posts `kSC4MessageOccupantHighlightChange` = **`0xE6E85114`** (posted from `0x80D600`). ⚠ **CORRECTED 2026-08-24 — this row read `0xA2D1C5B9`, which is actually `kSC4MessageOrdinanceAvailable`.** The `.data` name table pairs are `(guid, name-ptr)` and the original read was off by one row, so ANY id in our notes taken from that table by the same method is suspect. **The tint map is now fully decoded** (register #25): a runtime vector of 11 RGBA quads at `[SC4DrawContext+0x54]`, `tint = base + mode*16` at `0x7D4FCC`; mode 7 (query) = dark green `(0,.37,0,1)`, mode 5 (demolish) = dark red `(.66,0,0,1)`. `SetHighlight` = occupant vt+0x44 = `0x5E90E0`, writing the mode into the HIGH NIBBLE of byte `[occupant+0x2E]`. Vtable `0xA901A0` is the DEMOLISH control's | n-a (the tint covers exactly the model) |
...
| 15 | neighbor-connection arrows at city edges | **THE MARKER-OCCUPANT FAMILY (§2.5) — and the family's Rosetta stone.** Exemplar **`UI8x1x3_ConnectArrow_29F1` {0x6534284A, 0xC977C536, 0x29F10000}** carrying **OccupantSize {8,3,1} m** at exemplar bytes 0x58/0x5C/0x60, read by the marker factory via property **`0x27812810` @`0x4A25D3`**; binds its own S3D `{0x5AD0E817,0xBADB57F1,0x29F10000}` (244B on-disk/QFS-compressed, 336B decompressed — hand-decoded chunk-by-chunk in `row-15-neighbor-connection-arrows.md` §6, register #28); created by `0x6D4860` (push `0x29F10000` @`0x6D4A66`, +15.5f nudge `.rdata 0xAB1CA8`). **The name encodes the size**, but **the rendered size is NOT exemplar data** — see MEASURED 2026-08-31 below; it comes from the S3D model vertices — row 1's size, by contrast, is nine inline `imm32` floats inside `.text` | n-a (world units, no px imm, no zoom table on the path) |
| 16 | traffic/commute route overlay (query route trace) | **A DEDICATED `cISC4ViewObject3D`, built and registered by the traffic-query tool.** ⛔ THIS ROW PREVIOUSLY NAMED THE SIGNPOST-OCCUPANT MODULE AND THAT WAS WRONG — `cSC4SignpostOccupant` (ctor `0x5F5510`) belongs to the **Sign Tool**, not the query tool; the sizing chain quoted here was the sign's, not the route trace's. MEASURED CHAIN (confirmed live 2026-08-31 against a pre-stated prediction): pick handler `0x004D4D70` (tool vtable `0x00A90A88` slot +0x40), gated `0x004D4F19` on `[esi+0x8C]==1` (route mode) → `0x004CAC50` pick action, which first `RemoveViewObject`s any prior trace → `0x004CA460` builder: `new(0x2C)`, ctor `0x007DDD50`, primary vtable `0x00ABB648`, stored at `[tool+0x9C]` → `AddViewObject(obj, layer=5, key=0x3E8)` @`0x004CA54D`. **THE DRAWER IS `0x007DD9B0`** (vtable `0x00ABB648` slot +0x0C). Path result = a list of 0x44-byte strand nodes at `[drawable+0x14]`, filled per network tile by `0x004C5E20` via BeginStrand `0x007DDC50` / AddPoint `0x007DDB30` | **REACHABLE.** Scale lives at `[obj+0x10]`, written ONLY by `0x008B6DE0`; `0x007DD0D0` × 4.5 sets the dash step and `0x007DD410` divides item width by it. Six inline per-zoom immediates `{2,2,2,2,1.4,1.2}` @`0x004CA4A2` are the narrowest lever. Measured live: `[obj+0x10]` = **2.0** — but that is also the ctor default (`0x007DDD76`), so it does NOT confirm the per-zoom table. Dash cadence = `[obj+0x10] × 4.5` (`0x007DD0D0`, the 4.5 at `0x00ABB628`). Not patched — no defect reported at any tier
