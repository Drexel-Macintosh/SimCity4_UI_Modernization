...
| **a window DRAWS right but EATS or MISSES clicks** — part of its area is dead, a click lands on the wrong sibling, or a click hook is silent | one of five things, and they separate in a fixed order because each step rules out a family. **The engine's routing is first-claim-wins**: `GetChildWindowFromCursorPoint` (`0x0099DFA9`) walks the `[this+0x44]` list head-forward — the same list, direction and sentinel as `EnumChildren` — so **the first window in a tree dump that covers the point gets the click**, and it tests only `GetFlag(1)` and `GetFlag(0x200000)`: a **disabled** window claims exactly like an enabled one | **Work it in this order.** (1) Is the window in the tree dump at all, and where in sibling order? Anything earlier that covers the point wins — the dump index IS the priority. (2) Is slot 62 `[vt+0xF8]` the base `0x0099C97C`? If not, disassemble the override — a custom claim with a 1x width field is the classic "only the rightmost strip clicks" (six instructions beat six hours). (3) Is `WinFlag_MouseTrans` (`0x80000`) set? If not, the refined mask, `SelForce` and every buffer theory are irrelevant; if it is, the mask can only **subtract** from the rect test, never add — widen the rect, not the mask. (4) Silent downstream hook ⇒ hook the PARENT's slot 62 too: an upstream gate starves everything below, and that silence is not evidence the hook is wrong. (5) Only now hook a handler — read the `ret` immediate first (§1.4 arity). **⚠ The old step "compare `[0xA8]` against `[0x14]` for a stale absolute rect" is retired**: `SetArea` recomputes the absolute rect itself on every call and `GZWinMoveTo` tail-calls it, so the caches cannot disagree unless something poked `[+0xA8..+0xB4]` raw (`SDK-GAPS.md` §1.1) | disaster flyout (container `[0xE0]` = 49 against a 2x draw, `288−49` = the measured dead edge); the v2.11.16–18 sequence, where `DS149 count = 0` while `DS62` fired located the gate one level up |
...
| **the scale selector is MISSING from Options → Graphic Options at 1x** — the player has no in-game way back up to a scaled tier, and the log reads healthy. `README.md` sends every player to this panel, so it is the mod's most player-visible surface | **FOUR causes. THREE HAVE SHIPPED AS DEFECTS; THE FOURTH IS CORRECT BEHAVIOUR — check that one first.** (a) **`SelectorAtStock=0` — NOT A BUG.** The two ways of being at 1x want opposite things and `_tests\Set-Tier.ps1` says so in code (grep `wantSel`): `-Tier 1` writes `0`, because a stock reference capture must be a true no-DLL control, while every other tier writes `1` so a player who picks 1x in-game keeps a way back. A rig that last ran `-Tier 1` *should* have no selector. Shipped default is ON (`Settings.h`, `spikeSelectorAtStock = true`), and the release bundle ships **no ini at all** — an absent ini means ON, not OFF. (b) **the want was RECORDED but never COMMITTED** — v4.5.1 welded the commit to the tail of `SyncStaticLayers`, and `SyncSelectorPackage` records *after* that call returns on every path, so the want landed in a pool nothing would commit; at the stock tier the package could not arm and 1x was a one-way door. (c) **the code half ran and the data half was stashed** — 2026-08-19: `SyncSelectorPackage` lived *inside* `SyncStaticLayers`, which is not called at the stock tier, so the one package that exists only for the stock tier could never arm in the exact state it exists for. (d) **the payload was never bundled** — `Build-Dist` derives the bundle by parsing the deploy script and is blind to expression-built `Copy-Item` lines; SelectorUI was that blind spot's FIRST casualty | **The log's `CommitArming:` line is disk truth; the `want RECORDED` line above it is not.** `ScaleTier::SyncSelectorPackage` only RECORDS; `ScaleTier::CommitArming` → `CommitArmingImpl` is the single pass that touches disk; and the DIRECTOR owns that call on **both** boot paths, because only the maker of the boot's LAST record can know when recording is finished. ⚠ **SUPERSEDED WORDING, kept so it is not re-derived:** v4.5.1's log line said the selector was **ARMED** over a package that could not arm — a healthy-looking log above a stashed dat. The source now emits `SelectorUI-1x want RECORDED`; treat any older transcript quoting "armed" as unreliable. ⚠ **SUPERSEDED FILENAME, same reason:** the 2026-08-19 disk symptom was `z_SC4UIScale_SelectorUI-1x.dat.x1-disabled`, but that rename layout was replaced by the `.uipay` payload layout in v4.5.x — grepping disk for `x1-disabled` today finds nothing and **proves nothing**. Read the CURRENT names: `Plugins\zzz-SC4UIScale\z_SC4UIScale_SelectorUI.1x.uipay` and `.off.uipay`, staged onto a live `.dat` by `ArmOne` (grep `MISSING PAYLOAD` in `src\ScaleTier.cpp`, which falls back to `.off` — inert is the only safe wrong answer). Servicing, once armed, is `UiSpike::ServiceScaleSelector`, driven by the `WM_TIMER` the stock-tier branch installs | 2026-08-19 (neighbour gate — 3rd recording of that shape after #149/#182) · v4.5.1, fixed v4.5.2, USER-CONFIRMED on screen 2026-08-30 · `_packaging\Build-Dist.ps1` (grep `SelectorUI`). **Provenance: MEASURED 2026-08-31** |
...
and lists one the game does not have. The hooks are installed by INDEX, which
is why they land correctly; only the header's names are shifted — and
**corrected 2026-08-30, that shift is a BAND, not a tail**: taking
header-implied slot = declaration index + 3, the names are right at or below
real 56, one slot low from 57 to 133, **unusable across 134–138** (six
`GZOnMouse*` names for five slots), and right again from 139 up. `SDK-GAPS.md`
§1 carries the anchor table. Never name a mouse slot by counting the header.
