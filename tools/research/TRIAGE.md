...
| 2 | Do the two differ in **LEFT/TOP PARITY**? At f=1.5, `l*1.5` is integral only for even `l`. **ASK WHICH POPULATION THE CONTROL IS IN BEFORE ASKING ABOUT PARITY. (a) RUNTIME-SWEPT: `ScaleSubtree` is edge-derived for CONTAINERS ONLY. A LEAF (`GetChildCount()==0`) takes its EXTENT size-derived, `ScaleRound(w,f)`, logging `LEAFSIZE` (`src\UiSpike.cpp:17314-17338`), and the state-strip button class does the same (`src\UiSpike.cpp:17271-17281`) — so parity costs those no pixel of WIDTH. It still decides POSITION, which stays edge-derived for every window, leaves included (`src\UiSpike.cpp:17238-17239`). (b) PRE-SCALED: under a `kDataScaledSubtreeIds` root (for example the advisor strip `0x6A15C767`, `src\UiSpike.cpp:5373-5374`) `ScalePanelRoot` scales the ROOT and then RETURNS before the child loop (`src\UiSpike.cpp:14557`), so no descendant is ever swept and the parity rule there is the BUILDER's — `build_selective_safe.py::double_subtree_areas`, art-leaf = no children + `image=` + no `imagerect` (`tools\selective-safe\build_selective_safe.py:704-751`). Nothing downstream repairs a number the builder writes.** | `gate_btn_undercover.py` |
