...
- **R22 — Sub-flyout container height from item metrics.** `src\UiSpike.cpp` (grep `stripH = count*(cell 44 + gap 5) - 5` — the SUB-FLYOUT GEOMETRY block before the born-scale section) gives `H = max(count*(cell 44 + gap 5) - 5, 53) + 2*25`. The v2.34.0 constants path that patched the three provider sites in `sub_7EAEB0` and the seven container fields was retired and removed (audit 2026-09-25); the live lever is the born-scale path that follows that block. Not text (menu items are blits — grep `Menu items are BLITS` in the same block) but the same defect class, and `[0xE4] = 53` is simultaneously the bar width **and** the `IsPointInMe` claim width.
