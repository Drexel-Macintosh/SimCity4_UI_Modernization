------------------------------------
--Amry Truck - Launch Time- good
----------------------------------------
s = create_advice_citysituation('ac0adff9')
s.title = "text@6C15442E"
--
s.message = [[text@06C15414]]
--
s.priority=tuning_constants.ADVICE_PRIORITY_LOW
s.automata_list = automata_groups.army_truck -- sit_constants.lua
s.target_sequence = {    -- adv_const.lua
   building_groups.ArmyMissileRange, 
}
--s.progress_text = { 
--}
--
...
--
s.success_text = [[text@EC154530]]
s.failure_text = [[text@2C154539]]
--
s.frequency = sit_constants.FREQUENCY_SHORT
s.trigger="(game.reward_instance_count(special_buildings.MissleTestingRange) > 0) and (game.reward_instance_count(special_buildings.ArmyBase) > 0) and (sc4game.sitmgr.get_success_count('8c151efd') > 0)" -- adv_game_data.lua
...
s.evil_twin = hex2dec('ac0ae247') --Escape town with the stolen Stinkbomb Warhead.
--easy
...
--
s.frequency = sit_constants.FREQUENCY_SHORT
s.trigger="(game.reward_instance_count(special_buildings.ArmyBase) > 0) and (sc4game.sitmgr.get_success_count('8c151efd') > 0) and game.reward_instance_count(special_buildings.DeluxePoliceStation) > 0"
