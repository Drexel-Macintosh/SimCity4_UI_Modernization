...
start=base+sva; buf=data[ra:ra+rs]
md=Cs(CS_ARCH_X86,CS_MODE_32);md.detail=True
targets=[int(x,0) for x in sys.argv[1:]] or [0xd0,0xd4]
lo=int(__import__('os').environ.get('LO','0'),0); hi=int(__import__('os').environ.get('HI','0xffffffff'),0)
seen={}
for anchor in range(0,8):
    off=anchor
    while off < len(buf):
        got=False
        for ins in md.disasm(buf[off:], start+off):
            got=True
