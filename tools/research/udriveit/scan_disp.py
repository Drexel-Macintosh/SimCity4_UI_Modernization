...
# linear sweep from several alignments to catch most instructions
seen={}
for anchor in range(0,16):
    for ins in md.disasm(buf[anchor:], start+anchor):
        for op in ins.operands:
            if op.type==X86_OP_MEM and op.mem.disp in targets and op.mem.base not in (X86_REG_ESP,X86_REG_EBP) and op.mem.index==0:
