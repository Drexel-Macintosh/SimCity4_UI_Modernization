...
- [SC4ResourceLoadingHooks DLL](https://community.simtropolis.com/files/file/36242-resource-loading-hooks-dll-for-simcity-4/) version 2.0.1 or later (for Exemplar Patching)
...
- Copy the file `submenu-essentials.dat` and optionally the individual submenu files anywhere into your Plugins folder.
  You may skip installing an optional submenu if it contains neither network items nor any sub-submenus.
  In that case, the buildings and Flora of that submenu will appear in the default top-level menu instead.
...
Add the properties `Item Submenu Parent ID` and `Item Button Class` to the catalog item Exemplar (not the submenu button Exemplar).
...
Exemplar Patching is now entirely implemented in the SC4ResourceLoadingHooks DLL alone and does not require the Submenus DLL anymore.
...
- [SC4ResourceLoadingHooks](https://github.com/0xC0000054/sc4-resource-loading-hooks) LGPL 3.0
