...
static uint32_t CreateCatalogItemList_InjectPoint = 0x78f028;
...
static uint32_t CreateCatalogItemList_ContinueJump_EarlySkip = 0x78f0b6;
...
namespace
{
	...
	bool shouldHideSubmenu(const uint32_t buttonId)
	{
		if (isTutorial) {
			return submenuLinks.contains(buttonId);  // hide all known submenu buttons
		} else {
...
submenuButton:
	...
	{
		return spCategorization->belongsToMenu(propHolder, menuId, isTutorial);
	}
...
skipSubmenu:
			...
			// The following check was disabled to allow flora items in submenus outside the top-level flora menu.
...
skipSubmenu:
	...
	void NAKED_FUN Hook_CreateCatalogItemList3(void)
	{
		__asm {
			...
			push eax;  // store
			push edx;  // store
			push ecx;  // argument for shouldHideSubmenu
			push ecx;
			call addToItemList;  // (cdecl)
			add esp, 0x4;
			test al, al;
			jnz skipEmptinessCheck;
			call shouldHideSubmenu;  // (cdecl) note that here we assume that IID and Button ID are the same
skipEmptinessCheck:
	...
	bool isNonReachableFloraItem(cISCPropertyHolder* propHolder)
	{
		auto property = propHolder->GetProperty(ITEM_BUTTON_CLASS_PROP);
		if (property != nullptr) {
			uint32_t buttonClass;
			property->GetPropertyValue()->GetValUint32(buttonClass);
			if (buttonClass == ITEM_BUTTON_CLASS_FLORA) {
				property = propHolder->GetProperty(ITEM_SUBMENU_PARENT_ID_PROP);
				if (property != nullptr) {
					uint32_t submenuId;
					property->GetPropertyValue()->GetValUint32(submenuId);
					return !reachableSubmenus.contains(submenuId);
				}
			}
		}
		return false;
	...
	void NAKED_FUN Hook_CreateCatalogItemList(void)
	{
		__asm {
			...
			// check if we're in the top-level flora menu and the current item is a flora item from a non-reachable submenu, so should be displayed in this top-level menu instead
			cmp dword ptr [esp + 0x5c], TOOL_PLOP_FLORA;  // param_4
			jne checkMenuMatches;
			cmp dword ptr [lastVirtualButtonId], VIRTUAL_SUBMENU_ITEM_TYPE;  // check if nested
			je checkMenuMatches;  // only add non-reachable flora items in non-nested case
			push eax;  // store
			push ecx;  // store
			push edi;  // propHolder
			call isNonReachableFloraItem;  // (cdecl)
			add esp, 0x4;
			test al, al;
			pop ecx;  // restore
			pop eax;  // restore
			jnz addItem;  // flora item is not reachable, so add it

checkMenuMatches:
			// regular check if menu matches
			cmp eax, ecx;
			jnz earlySkip;
...
earlySkip:
			push CreateCatalogItemList_ContinueJump_EarlySkip;
			ret;
		}
	}
...
public:
	...
	void initializeSubmenusFromBuilding(cISC4BuildingDevelopmentSimulator* buildingDevSim, uint32_t buildingType)
	{
		...
		buildingDevSim->GetBuildingKeyFromType(buildingType, key);
	...
	void initializeSubmenus()
	{
		...
		cISC4RegionalCity* pRegionalCity = pApp->GetRegionalCity();
		if (pRegionalCity && pRegionalCity->IsTutorial())
		{
			logger.WriteLine(LogLevel::Info, "Detected a Tutorial city, so submenus are deactivated in this city.");
			isTutorial = true;  // in particular, treats every submenu as empty, so hides submenu buttons
		} else {
			isTutorial = false;
		}
			...
			if (pResourceList->Size() > 0)
			{
				pResourceList->EnumKeys([](const auto& key, void* pContext) {
					...
					if (propValueBuffer == ITEM_BUTTON_CLASS_SUBMENU && itemGid == ITEM_EXEMPLAR_GID_MISC_CATALOG)
					{
						...
						{
							submenuLinks[buttonId] = parentId;  // found valid submenu button, store it for later use
		...
		for (auto&& p : submenuLinks)
		{
			...
			if (isEmpty) {
				if (emptySubmenus.empty()) {
					logger.WriteLine(LogLevel::Info, "There isn't any content for the following submenus in your Plugins, so their submenu buttons will not be displayed in the menus:");
				}
	...
	bool PreAppInit()
	{
		static bool checkedExemplarPatchServer = false;
		if (!checkedExemplarPatchServer) {
			cIGZCOM* const pCOM = GZCOM();
			cRZAutoRefCount<cIExemplarPatchingServer> exemplarPatchingServer;
			Logger& logger = Logger::GetInstance();
			if (pCOM->GetClassObject(GZCLSID_cIExemplarPatchingServer, GZIID_cIExemplarPatchingServer, exemplarPatchingServer.AsPPVoid()))
			{
				logger.WriteLine(LogLevel::Info, "The Exemplar Patching functionality is available.");
			}
			else
			{
				logger.WriteLine(LogLevel::Error, "Failed to reach the Exemplar Patching server. Please install the SC4ResourceLoadingHooks DLL version 2.0.1 or later:\n\n  https://github.com/0xC0000054/sc4-resource-loading-hooks/releases\n");
			}
			checkedExemplarPatchServer = true;
		}
		return true;
	}
