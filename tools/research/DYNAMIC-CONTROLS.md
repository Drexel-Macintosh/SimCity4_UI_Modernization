# SC4 Dynamic (Code-Drawn) UI Controls — Class Map, Geometry Sources, Scaling Model

Binary: `SimCity 4.exe` 1.1.641.0 Steam (x86, 4GB-patched), ImageBase 0x400000,
`.text` raw==RVA 0x7000..0x680000, `.rdata` 0x680000, `.data` 0x707000.
Analysis is offline (capstone 5.x via Python 3.12).
...
1. **The exe contains a full `{clsid → class-name}` registry** in `.data` at VA 0xB08F78+
   (648 entries, pairs of `[clsid][ptr to name string]`). It names every custom clsid used by
   the .UI scripts.
2. **The RCI graph draws from its WINDOW rect** (fully proportional) — it follows a doubled
...
3. **TrendBar (poll bars / opinion bars) draws at its ART's pixel size**, centered in its
   ...
   TGIs that carry no `.UI` reference**, so a reference-driven package list cannot see them.
4. **The ticker (AdviceList) lays out its children from its own window size and the
   AdvisorHeadline font style** — a doubled container plus a 2x FontStyle makes its per-frame
   "geometry reset" produce correct 2x geometry by itself. The scaler policy for it is
   therefore "scale the container only, never its children".
...
## Class map (city HUD scripts I-2bc90671 composite, I-4bc906b5 / I-0a5fa5d6 polls, I-2a2aed99 ticker)
...
Other custom clsids in the registry: `0xca318388` cSC4WinMiniMap,
...
Two clsids are frequent in the scripts but absent from the named registry (framework classes
registered at 0x4662B0): `0xcbcbf1e0` (134 uses) is the U-Drive-It gauge dial, mapped in
"The U-Drive-It gauge class" below; `0xaa7cecfd` (56 uses) is not one of the dynamic controls
documented here.
...
## Geometry source per control (binary evidence)
...
The code cannot draw at stock size in a doubled window. If the bars read stock-sized in a
doubled frame, the three column windows themselves (0x09d27eb0/0x29d27ec0/0x49d27ed0, only
8x71 each) did not get resized in that run, or the 1x background art is what reads small.
Dumping the three windows' W/H after scaling settles it: 16x142 means the bars are correct
by code.
...
**six cells — `bandW = fillW/6`** (0x7BF0E4 `imul 0xAAAAAAAB` / 0x7BF0F5 `shr 2`, the /6
reciprocal), and places the fill marker at `fraction[this+0xE0] × (imgdim − 1)` — i.e. every
content dimension comes from the **image**.
...
then `[vt+0x10]` = SetImages). The .UI scripts never reference these TGIs, so a
reference-driven selective-art derivation is blind to them; they are carried explicitly in
`CODE_BOUND_TGIS` in `build_selective_safe.py`. The .UI-visible tick overlay 14015585 is
reference-visible and needs no such entry.

The fill is always proportioned **relative to its own groove image** (fraction × image width),
so the bar reads "correct" even when the whole groove+fill unit renders at 1x centered in a
2x frame, and doubled 14015585 tick-overlay BMPs on top reinforce that impression. Only the
groove+fill art size moves the unit.
...
- Item creation @ 0x7931F1: creates the internal roll window (CreateInstance clsid
  ...
  **`SetArea4(0, 0, GetW(), GetH())` of the AdviceList window itself** (vcalls `[+0xA4]`,
  ...
  therefore reproduces whatever size the container is; it only fights a scaler that touches
...
- Ticker init @ 0x77258B–0x772735: caches the marquee rect, fetches font style GUID
  ...
  **Strip height and scroll step both scale with the font style — no pixel constants.**
  (The community "ticker ignores font size" precedent applies to DAT-based FontStyle; the
  loose-file root FontStyle.ini is the probe-order winner per the FontStyle research, and the
  style is fetched live by GUID.)

### Funds / population / rating text — FONT-derived
...
(2x whole-file replacement deployed). Doubled window + doubled style = correct.
...
- groove BMP 0x8a517556: image (re)bound via cIGZWinBMP (iid 0xc12cea13) `[vt+0x10]`;
  groove art 14015549 is .UI-bound and 2x in the package (staged imagerect 204x22), but that
  staged imagerect is dead data at runtime. What is (re)bound is not the sheet but a
  ...
  before a resize keeps drawing the old size until the next tick. Full mechanism:
  "The SetImage crop latch" below and `SC4-UI-ENGINE.md` §2.6.
...
With doubled windows and 2x arrow art but an unpatched constant, the reveal shows **half**
the intended arrow cells. That artifact is purely constant-driven.

## Data-driven levers for CONTENT scale
...
| cSC4WinTrendBar | **art size** of code-bound TGIs 0x14015580 (groove) + 0x14015584 (fill), group 0x46A006B0, type 0x856DDBAC | **no** — code-bound, carried in the override .dat |
...
## Scaling model per control

| Control | Class of problem | How it scales |
|---|---|---|
| RCI demand graph | window-derived | Nothing to do beyond the window: at 2x the three column windows measure 16x142 and the bars follow |
| Opinion-poll bars (TrendBar) | data (art) | 2x overrides for code-bound art `{0x856DDBAC, 0x46A006B0, 0x14015580}` and `{…, 0x14015584}` ship in z_SC4UIScale_SelectiveArt.dat (2x-in-place, same TGI — the art is loaded from DBPF by TGI, so an override dat wins). Both are in the exclusive set of the selective build |
| News ticker marquee | data (font) + scaler policy | Double ONLY the marquee window 0xaa12f33c (plus panel/flatrect/BMP as normal windows) and **never touch AdviceList children**; 2x `AdvisorHeadline` in FontStyle.ini does the rest, since strip height and scroll step are font-derived. The double-scale guard on its children is permanent |
| Funds/population/date/rating texts | data (font) | Covered by the deployed 2x FontStyle.ini; a field that still reads 1x is a FontStyle probe-order question, not geometry |
| Mayor Rating change-arrows | code | In-place byte patch at DLL init on the three `imul …, 7` sites — bytes `6B F6 07` @ 0x7E87B1, `6B C9 07` @ 0x7E89D7, `6B C9 07` @ 0x7E8A02 — where the last byte carries the px-per-rating-point width (0x0E at 2x). Face art 0x14315e60/0x14315e62 ships as 2x overrides. The cached arrow base pos `[obj+0x378/0x37C]` is snapshotted at panel init, so it must be captured after scaling |
| News/advice window art | data (art) | More code-bound art constants live at 0x77A495–0x77A837 and 0x780952–0x78910C (instances 0x140155b4..0x140155f7, c8, cb, cc, d0–d7): the news window + advisor panels bind art by TGI in code |

## News content: the HTML text engine

The font style drives the marquee GEOMETRY (measured 676x90 = 3 × doubled
lineHeight live), and the item TEXT comes from the game's HTML renderer —
exe .rdata templates with literal `SIZE=2`/`SIZE=3` (reader headlines
0xA83850/0xA83820, story pages 0xAB57A8/0xAB5810, bold headers
...
style sizes: `idx = (4*size+8)/18` at 0x762F30 / 0x52CC70. SC4UIScale scales
both tables and retargets the popup style GUIDs at stock-size clones.
The art at 0x140155b4..f7 and the LTEXT sc4:// art are in SelectiveArt (328
entries). The rich-item class 0xaa12e5f5 lives at code ~0x8FA317
(GetClassID); creation sites 0x443FC9/0x76A182/0x78CE11/0x7931F0.

## Audio Options playlist grid checkboxes
...
instance ONE below the .UI-bound radiocheck strip 0x14416245). It is loaded by the
...
standard image loader `call 0x602B00`, so an override dat wins by TGI. It carries zero .UI
refs (corpus-verified) and is listed in `CODE_BOUND_TGIS` in build_selective_safe.py.
In-game at 2x both checked and unchecked states draw correctly, i.e. the grid's strip slicing
is proportional (imageWidth/8) with no pixel constants — same rule as GZWinBtn.

Two spellings in the same grid govern `build_dialog_static.py`: the grid attributes are
`drowheight`/`dcolwidth` (d-prefixed), so a `\browheight` pattern silently misses them and
rows ship at 20px inside a doubled dialog; and `wingridcol="1,1,200 "` carries a PIXEL column
width in every 3rd slot, so the width scales and the two index slots never do.
...
**Why an art pass keyed on `.UI` references cannot see it:** the .UI children carry NO
`image=`. The dashboard binder **0x005646AE–0x0056477C** reads VEHICLE EXEMPLAR
...
**The cure:** a per-instance vtable-copy hook on slot 88,
...
src reads past the texture and produces a tiling mess), restored
immediately after. It is self-limiting to the live window, so an unscaled window
or already-2x art degrades to a no-op.

## The SetImage crop latch

**Mayor Rating groove (GZWinBMP 0x8a517556).** sub_7E8510 **composes** the fill buffer per
...
city sweep in 61/61 captured sessions, so the crop latches at the 1x size and
heals only on the next sim rating tick. THE LATCH LAW: a latch computed
from live geometry is a hidden consumer of that geometry — the question is WHEN content
was BOUND, not what the geometry is now. The cure is RELATCH in
`src\UiSpike.cpp`, armed per panel root and keyed on the latch signature.
Consequently the groove's script-side `imagerect` is not a live lever on that window, and
the three `imul ,7` sites are ARROWS ONLY — **no pixel constant exists in the fill chain**
(measured tick pitch at row 5 is 4px, not 7).

**cSC4WinTrendBar (polls bars) is IMMUNE to the latch** — Draw 0x7BF0A0
...
SetImages (0x7BEEB0, main vt 0xABA68C slot 4) stores POINTERS only. The full
member census shows zero stale-able geometry; bind-before and bind-after the
...
The fill sheet {46a006b0,14015584} is a **six-cell strip**: states=6 is carried in
`find_cell_strips.py`'s CODE_BOUND table because zero `.UI` refs exist for the
`.UI`-derivation to see.
