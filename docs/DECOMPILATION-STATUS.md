...
the canonical headline **86/117 distinct root ids = 73.5%**, re-measured ⛔ SUPERSEDED 2026-09-01: this is no longer the canonical headline. The metric counted only ONE of the mod's delivery mechanisms - a literal in `UiSpike.cpp` - and missed the staged dialog path entirely, undercounting by seven player-facing dialogs that ship PRE-SCALED. Counting both mechanisms gives **93/117 = 79.5% of ALL stock .UI roots**, and removing the roots the retail game cannot instantiate (each excluded with a named mechanism and a positive control) gives ~~**93/93 = 100%**, on a **24**-row exclusion set~~ ⛔ SUPERSEDED THE SAME DAY, 2026-09-01. **Why it was retired, not merely restated:** that 24-row set removed only roots that were ALREADY outside the numerator, which is why it printed 93/93 with the numerator untouched. The re-derived set is **27** rows, and the three it adds are the dead *Move In My Sim* design variants `0x6BFAC122` / `0x8BFAC13E` / `0xCBFACAE1` — never created, yet each named once in `src\UiSpike.cpp`, so they sat INSIDE the numerator. MEASURED 2026-09-01: of all 27 excluded ids those three are the only ones `grep` finds in `src\UiSpike.cpp` (*positive control:* the same grep over the same file returns one hit each for them and zero for the other 24, so the null is a real absence). Excluding them therefore moves numerator and denominator together, 93/93 → **90/90 = 100% of RETAIL-REACHABLE stock .UI roots**. MEASURED by `python tools\uimap\coverage_rederive.py`, which the banner of `tools\uimap\coverage-matrix.md` names as "the ONLY tool allowed to state a coverage figure"; it prints the whole exclusion table every run and then `denominator  117 total - 27 unreachable = 90 retail-reachable`, `numerator    90 reached by either mechanism`, `==> 90/90 = 100.0% of RETAIL-REACHABLE stock .UI roots`. The 27 break down as lot-editor 11, debugger 5, exemplar-editor 4, dead-variant 3, and four singletons (dev-twin, phantom, `0x8A10BA95` Simulator Control — "nothing in the retail build can name the script" — and unsweepable). That scope phrase is part of the number: "100% of the UI" would be false while the unbounded code-created channel exists, and **93/117 = 79.5% over ALL roots** is kept alongside it so the 27 exclusions stay auditable. ⚠ **That run exits `OVERALL: FAIL` and these lines are still quotable:** all six divergences are the tool's own CORPUS-SIZE class (`[FAIL] ui files on disk  expected 339  measured 1093` — a plugin set larger than the baseline), while the check this denominator rests on reads `[OK ] distinct nonzero root ids  expected 117  measured 117`.
...
coverage figure". ⛔ **D-2's VERDICT SUPERSEDED 2026-09-01 — both halves are now false, and the second one is discharged.** The struck sentence is kept because it was the honest reading when it was written; a reader must be able to see what was believed and why it changed.

~~"The register's §A.5 numbers are not wrong for their scope but are **not the canonical headline**, and §A.5 does not say so."~~

**Half one — "not wrong for their scope" — is REFUTED. Both D-numbers were wrong inside their own scope, not merely quoted at a loose denominator.** `tools\uimap\coverage-matrix.md` §4 ("GENUINELY UNMAPPED — 10 root slots, 9 distinct ids") has carried a 2026-08-16 instruction to "Re-derive '10 root slots / 9 distinct ids'" before quoting either. It was executed for the first time on 2026-09-01 (`_tests\REGRESSION.md`, grep `D1 re-derived: all nine gaps were already closed`). All nine gaps had closed — six genuinely, and **three were being counted COVERED without being covered**: `0x6BFAC122`, `0x8BFAC13E`, `0xCBFACAE1` sit in `kOwnsBackgroundSheet`, a DERIVED array they qualify for on art data alone while the windows are never created. ⇒ ~~D1 = 288/298 = 96.6%~~ **RETIRED — it certified three non-instantiable roots as covered.** D2 was re-measured the same day to **13 of 17 code-created named windows = 76.5%** (grep `D2 is 13/17 = 76.5%, not 11/17 = 64.7%`); `0x00000043` Restore-Toolbars and `0xEA659793` region screen had gained coverage. ⇒ ~~D2 = 11/17 = 64.7%~~ **RETIRED as stale**, and ~~combined 299/315 = 94.9%~~ **RETIRED as arithmetic over a wrong D1 and a stale D2.**

**Half two — "and §A.5 does not say so" — is DISCHARGED.** `research\UNKNOWNS-AND-NEXT-TARGETS.md` §A.5 now opens with a ⛔ **UPDATED 2026-09-01** banner saying the bullet list below it predates the current denominators and naming a canonical one; the three-denominator bullet itself is kept as written, which is what annotate-never-delete requires. The drift this item reported is closed in the register. **Nothing is owed there by D-2.**

**THE CURRENT FIGURES, EACH WITH ITS SCOPE.** Printed by `python tools\uimap\coverage_rederive.py`, run 2026-09-01 — the only tool allowed to state a coverage figure. Do not hand-compute either.

- **90/90 = 100.0% of RETAIL-REACHABLE stock .UI roots.** The tool prints `denominator 117 total - 27 unreachable = 90 retail-reachable`. ⛔ **The scope phrase is part of the number. "100% of the UI" is FALSE and must never be written** — this denominator does not touch the unbounded code-created channel.
- **93/117 = 79.5% over ALL roots**, kept so the 27 exclusions stay auditable. The tool prints every excluded root with its mechanism on every run (Lot Editor 11, Lua debugger 5, exemplar editor 4, three dead Move In My Sim variants, Simulator Control, and three singletons).

⛔ **The annotation earlier in this item is ITSELF superseded — do not quote its `93/93` or its "24 roots".** `MAX_EXCLUSIONS` went 24 → 27 when the three fake-covered roots were excluded; because those three had also been in the numerator, it fell by three alongside the denominator and the ratio held at 100%. **A ceiling raise that costs the numerator is the only kind that cannot be motivated by wanting a better number.**

*Run condition, stated because a reader who re-runs it will see this:* the 2026-09-01 run printed `OVERALL: FAIL - 6 hard expectation(s) diverged`. Every divergence is in the corpus-size class — this machine has plugins installed, so `ui files on disk` measured **1093** against an EXPECT of 339 — and those inflate the raw root counts, not the 117 distinct stock-id denominator these figures rest on. The check that headline does rest on read `[OK ] distinct root ids named  floor 83  measured 86`. Per the tool's own banner, quote no number whose own check FAILED; none of the figures above is one.
