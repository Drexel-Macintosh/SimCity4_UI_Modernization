# SC4 UI-Scaling â€” TEST SCENARIO MATRIX

This file exists because one session produced **five separate bugs each
caused by an untested scenario axis**, not by bad code. Every fix passed in
the scenario it was built in and failed in another.
...
## Why this file exists â€” the ones that bit us
...
| Disaster thumbnails tiny (v2.39.0) | every window still owned by the sweep | **the one window we moved to born-at-Place** â€” it silently inherited item metrics from the sweep it no longer visits |
| Picker icons duplicated GAME-WIDE (v2.39.1) | the panel we were working on | **every other panel in the game** â€” one window's birth wrote a scaled value into a SHARED latch |
...
## THE MOST EXPENSIVE AXIS: 1.5x IS NOT "2x BUT SMALLER"

The first eyes-on pass at 1.5x found **four** real defects in one session
that every automated gate had passed at all three tiers for weeks.
...
can be silently wrong at 1.5x â€” and the integer tiers cannot express the defect
...
| the game's `width/3` and `width/4` **cell divides** | `N \| v` implies `N \| k*v` for integer `k` â€” divisibility is preserved for free. At 1.5x: **31%** and **43%** of dimensions break | #143 |
...
breadth â€” for this whole class, the fractional tier is the *only* tier with the
power to disagree.
...
enabled, `SelectiveArt-15x` went 10.5 MB â†’ 20.5 MB against 2x's 11.4 MB. A
...
**UI scale and TEXT scale ship LOCKED 1:1.** Settled by the user: text is
**not** a user-facing setting, shipping UI values are **1.5 / 2 / 3 only**,
and above 3x is not supported. So this matrix deliberately has **no
text-scale axis** â€” and if one is ever proposed, **adding it here is part of
the change, not a follow-up.**

The justification is measured, not stylistic: `tools\research\SCALING-AXES.md`
Â§2.2 catalogues **26 places** where a box is sized by its text or a text is
wrapped to its box. At `t â‰  u` each of those pairs silently unpairs â€” and
...
(`B â‰¤ 2 Ã— UI`; drawn 96/128/192 px at UI 1.5/2/3). Its ceiling is set by the
**art**, not by clipping â€” the marker's parent is the full-screen 3D view, so a
...
- **A box that must contain RENDERED TEXT is not a `FACTOR` constant**
  (law 48, #57). Ink grows Ã—2.13 per doubling, not Ã—2.00, so `round(stock Ã— f)`
...
## AXIS 2 â€” Mod state (this is the axis that hurt most)
...
| Quit/exit confirms unmodded | `Toggle-SaveWarningUI.ps1 -Off` / `-On` | the two states must BOTH be flawless |
| Vanilla | `Set-StockPlugins.ps1` / `-Restore` / `-Status` | the vanilla verification pass |

**`Set-StockPlugins.ps1`** is BROADER than `Set-StockCompare.ps1` (which
disables only OUR layer): it stashes every loadable file in the Plugins root
(`*.dll`, `*.dat`, `FontStyle.ini`) and every content subfolder â€” NAM,
CAM/Maxis, the null45 + memo DLL families, MoreBuildingStyles, thumbnail and
texture fixes, **and our own `z_SC4UIScale_*.dat` + `zzz-SC4UIScale\`**. The
rule is by SHAPE, not a hand-list, so a plugin we have never heard of is
still caught. It also takes the INSTALL-tree `FontStyle.ini` copies â€”
`<install>\Plugins`, `<install>`, `<install>\Apps` â€” the exact three-probe
contaminant that made earlier "stock" captures Franken-captures. Nothing is
deleted; a manifest drives an exact `-Restore`. It also sets a windowed
resolution, default 1024x768, `-Width`/`-Height`.

**The stash is a SIBLING of `Plugins\`, never a child** â€” `Documents\SimCity
4\_stock-stash`. SC4's plugin scan is RECURSIVE: the first version of this
script stashed INSIDE `Plugins\` and 132 dats (98 MB) + 30 DLLs stayed live
through a whole stock-baseline investigation. Do not "tidy" it back inside,
and do not hand-roll a replacement that repeats the bug.

**`SC4TouchControls` is NOT kept.** It is stashed on every run, and
`-Restore` REFUSES it by name under the standing quarantine order.
`-IncludeTouch` is **not** an escape hatch and is read nowhere in the stash
logic; ignore any stale summary line claiming otherwise.

DELIBERATELY KEPT: `SC4GraphicsOptions.dll`/`.ini` â€” this IS the resolution
lever â€” and dgVoodoo's `DDraw.dll` in the GAME INSTALL dir, which is not
touched at all: SC4 on Win11 needs it to start, so removing it risks not
launching rather than launching stock.
...
2026-07-31 â€” with CoriBoom's mod deleted, our copy (532x640) still beat the
...
`python tools\dbpf\who_owns_tgi.py <instance...>` â€” it prints every holder in
...
- Build third-party overrides from **the MOD's** script/art, never the stock
  one â€” different dimensions, and using stock silently reverts the mod's
...
- Wrong text COLOUR was a symptom of the wrong SCRIPT being loaded â€” not a
...
## AXIS 3 â€” Game mode / screen

Verified distinct behaviour in each; a fix in one proves nothing about another.

| Mode | Setup | Notes |
|---|---|---|
| Region view | launch â†’ region | separate scaling path (`ScaleRegion`, timer-polled: no city message fires) |
...
| City, GOD mode **founded** | founded city â†’ god | **different toolbar (`0xC991EDA8`), 120px pitch.** Several windows measured "hidden/inert" pre-founding go LIVE once a city exists |
| Sim mode / My Sims | enter Sim mode, open the My Sims catalog AND a Sim detail page (profile / actions / find-sim) | **FULLY LIVE â€” a real axis, not out of scope.** The family carries four mechanisms: 2x art via `SCALED_WINDOW_IDS` â€” which lives in **`tools\selective-safe\build_selective_safe.py`**, not in UiSpike; pre-scale while hidden via `kAlwaysScaleCityIds`; dashboard design-x via `kCityHudFamilyIds`; and the GZWinBMP runtime-portrait draw hook via `kBmpxCityRoots`, which is what ANSWERED the old "slot-pitch code hook" concern â€” that concern was the portraits tiling inside doubled slots, closed as task #47 in v2.42.4, user-confirmed. **Test BOTH halves:** script I-aa1f1f57 has NINE marker-composed top-level roots, not three; an early pass covered the CATALOG side only and left the whole DETAIL side 1x against 2x siblings |
...
## AXIS 4 â€” Panel lifecycle (the sneakiest axis)
...
| **FIRST open per city load** | the game BINDS things once here: 3D advisor heads framed from the then-current geometry, strip fields captured once. **Our sweep cannot run earlier than this bind** â€” so anything bound at city load must be fixed in DATA |
| Re-open | usually correct even when first-open is broken â€” which is exactly why first-open bugs survive testing |
| Compact vs EXPANDED | different child subtrees, often different art (Building Style, Budget) |
| After a city SWITCH | **the game REUSES window objects across cities** â€” scale records must never be cleared between cities |
| Region â†” city switch | re-entry paths differ; check both |
...
Moving a window to **born-at-Place** does not just make it earlier â€” it takes
...
port **all** of it â€” a promotion is a migration, not an optimisation.
...
latch from the **stock** argument, never from the scaled one â€” v2.39.1
...
switch â€” verify the key name matches *exactly*, not by substring), and (b) one
UNRELATED panel of the same family opened afterwards in the same session.

## AXIS 5 â€” Render mode / resolution
...
non-desktop modes â€” `ForceDrawOnScroll=true` fixes it.

## AXIS 6 â€” Input path

Mouse vs touch (the separate `SC4TouchControls` plugin) vs our synthesized
...
- **Alignment markers** (`id=0x0000AAAA`) are POSITIONING DATA. The game
  places a panel at `anchor âˆ’ markerOffset` in NATIVE units. Scaling one
  shifted the whole Advisors box by exactly `âˆ’(229,63)`. Not at runtime, and
  not in shipped data either.
- **Font-sized / art-sized controls** â€” a control whose size is computed from
  ...
  down arrow).
- **AdviceList children** â€” items are game-sized to the container; recursing
  double-scales them.
...
machinery") that a tree dump disproves in one read â€” it is the Data Views
...
copies share its root id) â€” the I-898897de lesson, now proven twice.
...
repositions with 1x metrics, then native death â€” suspect the code-painted
...
poisons them (writing strip fields from the sweep captured 88 as natural â†’
forced 176 â†’ 4x pitch everywhere). Sweep-side code may INVALIDATE, never write.
...
**Alpha guard shape is `0 < a < 128`, never `a < 128`** â€” stock magenta-keyed
art has `a == 0` everywhere.

**Parse BOTH exemplar formats** (binary EQZB/CQZB *and* text) â€” CAM is ~half
...
- **The game runs ELEVATED.** A non-elevated shell cannot kill it
  (`Access is denied`). Never kill it anyway â€” use the wait-for-close deploy
...
- **OneDrive holds directory handles** â€” `shutil.rmtree` deletes the files then
...
- **`[Probe]` keys are NOT live-tunable on a default install.** The whole ini
  re-read sits behind `[UiSpike] LiveTune`, which ships **0**: the block runs
  ONCE on the first sweep and then never again. Every `[Probe]` key â€”
  `Enabled`, `BandL/R/T/B`, `Max`, `EdgeDump`/`VisTrace`/`EdgeBlt`, `ThinBlt`
  and the `Icon*` family â€” is read inside it. The statics are function-level
  in `ScaleGodFlyouts`, so this is once per **game launch**, not per city.
  `LiveTune` is itself latched on that same first pass â€” flipping it
  mid-session does nothing either. So: set `[UiSpike] LiveTune=1` **before**
  launching to restore the 20-sweep poll, otherwise restart the game after
  editing `[Probe]`.
  This is the instrument-null trap: arm a probe mid-session, see no lines,
  and conclude the code path never runs. Note `LiveDumpMs` is read once at
  startup and DOES need a restart regardless.
...
- Log files are share-locked while the game runs â€” read with a
...
2-3 builds and twice broke something that worked. The proof:
...
- The spinner: the probe gave 60x72 inside a 98x44 parent â€” the clipping was
  arithmetic, not opinion.

**If two symptoms contradict each other, you are at the wrong LAYER â€” move up
one.** Build the instrument, then read it.

---

## AXIS 9 â€” LAYOUT DENSITY (cost four regressions)

**A geometry change is not one test. It is one test PER DENSITY.**

The same edit â€” move 177 buttons up to 2px onto an even edge â€” produced:

| layout | density | result |
|---|---|---|
| Landscape flyout (`09923283`) | 5 buttons, ~50px apart | **invisible** â€” reported as fixed |
...
**ID NOTE.** The 21-face grid is in `I-0a243d80`, `caption="Select A My
Sim"`: 22 buttons of 50x49, of which 21 sit at columns x=59,111,163,215,267,
319,371 and rows y=36,86,136 â€” the 22nd, at (68,243), is the selected-sim
preview. **All seven of those columns are ODD and all three rows EVEN**, so
the reverted parity nudge (q=2 at 1.5x) moved all 21 faces sideways and none
vertically â€” exactly the user report, "all the faces are shifted to the
left". Do not confuse it with `I-aa1f1f57`, captioned **"My Sims"** â€” the
nine-root catalog/detail family â€” whose densest button row holds **four**
buttons, not twenty-one.

The **24 and 28** nudge counts quoted in the builder belong to that other
script, `I-aa1f1f57`, which ships as two group copies (`G-08000600` and
`G-96a006b0`), hence two numbers. `I-0a243d80` exists as a single file and
cannot produce a pair. **The most-nudged script and the panel that showed
the defect are not the same window** â€” testing on either one alone is the
...
2. Look at anything with a **frame around a grid** â€” a 1px shift is invisible in
...
## AXIS 10 â€” WHO CREATED THE WINDOW
...
|---|---|---|
| edit `area=` in a `.UI` | yes | no (nothing to edit) |
| edit ART dimensions | yes | **yes — the trap** |
...
**Test any art-dimension change against a runtime-created consumer** â€” the
disaster flyout thumbnails, hovered â€” before believing an offline gate.

## AXIS 11 â€” WHOSE DIALOG IS IT?
...
|---|---|---|---|
| stock dialog | yes | â€” | `DialogStatic` (or the runtime sweep) |
| stock dialog a mod REPLACED | yes | yes | `TP_TARGETS`, built from the mod's script, mod-gated |
| **a dialog the mod ADDED** | no | no | **nothing, until someone notices** |
...
that row for the **entire life of the project**, rendering at 1Ã— under 1.5Ã—
...
query panels). All three are now scaled â€” but the next content mod's own
