...
| ~~9~~ | ~~**Third-party / Maxis derived-content audit** (task #146)~~ **CLOSED 2026-08-19 by user decision - no issue.** | Every shipped `.dat` contains upscaled Maxis art. `NOTICE` and `THIRD-PARTY-NOTICES.md` are thorough about *code*; the position on *art* needs to be stated in one sentence a moderator can read | needs a decision, not a build |
