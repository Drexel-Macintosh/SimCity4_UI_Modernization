...
| `Test-DatIntegrity.ps1` | every deployed dat's entry count, the 3 font sources, the DLL's presence and the Plugins-folder quarantine check, plus a DEPLOYED==BUILT content hash for every package with a canonical build output. A package left out of `_packaging\PackageFiles.psd1` (the list `Deploy-OnGameClose.ps1` copies from) rots silently — stale and fresh dats can have IDENTICAL entry counts and byte sizes, because a reference rewrite swaps equal-length hex strings, so only a content hash catches that class. **The one to run after any data change or deploy.** Its DEPLOYED==BUILT pairs and font sources come from `_packaging\PackageFiles.psd1`; add a new package's entry-count rows here in the same commit (a listed package without one is reported, and a count row for a package the list does not deploy is red). It FAILS between build and deploy by design: deploy, then re-run. | ~1 min |
| `New-SyntheticPlugins.py` | **WRITES N FILES** (`--files`, default 1000; the boot harness runs it at 50,000) under `C:\dev\_scale\Plugins-<N>` — a synthetic sc4pac-shaped Plugins tree with KNOWN ANSWERS in `manifest.json`: our own release bundle copied in as "ours", every `kThirdPartyDeps` filename planted (present at the pinned size / present at the WRONG size / duplicated in two folders / ABSENT), NAM's controller 3 or 4 dirs down (`--nam-depth`), the web-button dat behind a 390-char path (`--long-paths`), a 200,000-entry DBPF (`--big-index`), an OFFLINE-attributed file and a Plugins-into-Plugins junction loop on request. Every expected count is MEASURED from the tree after it is written. It refuses OneDrive, `Documents\SimCity 4` and the game install, deletes only a tree carrying its own manifest marker, and reuses a tree whose parameters match. Never touches the game. | seconds (1,000) to ~1 min (50,000) |
| `Test-BootWalk.ps1` | **WRITES N FILES per `-Files` value** through the generator above (three trees by default: the requested N plus two 60-file controls). Lifts FOLDER-DISCOVERY, DEP-WALK and BOOT-WALK (and BOOT-INDEX when the source has it) out of `ScaleTier.cpp` by sentinel, compiles them x86 with the DLL-side symbols stubbed and `FindFirstFileW`/`FindNextFileW`/`CreateFileW` wrapped in counters, then runs the boot walks against the trees and prints a flat `phase ms count finds nexts opens` table. Every count is checked against `manifest.json`; the three EXPECTED failures of the pre-4.10.0 code (icons in a >=200,000-entry index dropped; a dependency past MAX_PATH invisible — and, measured, the bare walk TERMINATES THE PROCESS with 0xC0000409; NAM at depth 4 absent) are reported as positive controls, never hidden. `-Source <file|git-ref>` lifts from a blob (`3f2608b` = the pre-rework baseline), `-Record` writes `golden\bootwalk-baseline.json`, `-Mutate` shallows the walk by one level and REQUIRES the depth-3 needle to vanish. Never touches the game. | ~1 min at 1,000; ~4 min at 50,000 (first generation included) |
| `Test-ProbeDerefGuards.py` | Source gate: no speculative pointer read in probe/detour scope outside `__try`/`ProbeSafe` (4.10.0; `--selftest` 17 cases; `// deref-ok: <reason>` waives a hooked-call own-object read into a printed list). RED = a crash class shipping again.  | ~2 s |
| `Test-GZWinHeaderSlots.py` | **Allowlist gate.** Compiles a probe against the vendored gzcom-dll `cIGZWin.h` with MSVC and compares each method's compiled slot with its slot in the exe. Each exe slot was decoded from the function's own code in the base vtable; many were also compared across 15 window classes. The header puts 40 of its 147 declarations on the wrong slot: 47-50, 53-57, 102-107 and 118-147 (e.g. `GZWinMoveTo` compiles to the exe's `GZWinOffset`). RED if `src\` calls a cIGZWin method name that is on the wrong slot, has one of the three known ABI defects, or has no exe-verified row. Also RED on a `GZWinMoveTo` whose arguments are not delta-shaped (waiver: `// relative-ok: <why>`), or if a control moves: GetID/GZPaint must compile to 63/88, and GZWinMoveTo/GetArea/SetSize/PlotPresent must still mismatch. `--selftest` plants `GetArea()`, `PullToFront()`, `SetShadeColor()` and `GZWinMoveTo(640, 480)` and requires each to fail. The scan is by name, not receiver type (limits in its docstring). Needs MSVC (vcvars32). | ~5 s |
| `CRASH-CENSUS.md` | Every game exception report on the dev machine, mapped (module, RVA, bytes at EIP, ledger status). Release checklist: a report younger than the last ledger entry blocks the release.  | read only |
| `Test-BigPlugins-InGame.ps1` | **LAUNCHES THE GAME** against a synthetic user dir (`-Root <dir>` holding a `Plugins` junction to a New-SyntheticPlugins tree; the game needs `-UserDir:"<Root>\"` WITH the trailing backslash - it concatenates without a separator, measured) and samples private/peak/working-set bytes every 2 s from an unelevated shell, DLL run vs `-Control` (DLL set aside). Prints the region-screen milestone, peak memory, and whether the user dir was honoured. A COLD 50k tree costs ~6 min in the antivirus first-open of every file before our code runs its 4 s. | minutes (warm) to ~20 min (cold 50k) |
