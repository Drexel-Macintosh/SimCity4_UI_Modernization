...
| `Test-GZWinHeaderSlots.py` | **Allowlist gate.** Compiles a probe against the vendored gzcom-dll `cIGZWin.h` with MSVC and compares each method's compiled slot with its slot in the exe. Each exe slot was decoded from the function's own code, on 15 window classes. The header puts 40 of its 147 declarations on the wrong slot: 47-50, 53-57, 102-107 and 118-147 (e.g. `GZWinMoveTo` compiles to the exe's `GZWinOffset`). RED if `src\` calls a cIGZWin method that is on the wrong slot, has a known ABI defect, or has no exe-verified row. Also RED if a control moves: GetID/GZPaint must compile to 63/88, and GZWinMoveTo/GetArea/SetSize/PlotPresent must still mismatch. Mutation-tested: an added `GetArea()`, `PullToFront()` or `SetShadeColor()` each exits 1. Needs MSVC (vcvars32). | ~5 s |
