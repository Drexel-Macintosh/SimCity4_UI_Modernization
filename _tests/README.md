...
| `Test-ProbeDerefGuards.py` | Source gate: no speculative pointer read in probe/detour scope outside `__try`/`ProbeSafe` (Beta 1; `--selftest` 17 cases; `// deref-ok: <reason>` waives a hooked-call own-object read into a printed list). RED = a crash class shipping again.  — |
| `CRASH-CENSUS.md` | Every game exception report on the dev machine, mapped (module, RVA, bytes at EIP, ledger status). Release checklist: a report younger than the last ledger entry blocks the release.  — |
