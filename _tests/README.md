...
| `Test-GZWinHeaderSlots.py` | Compiles a probe against the vendored gzcom-dll `cIGZWin.h` with MSVC and compares each method's COMPILED slot to its slot in the exe (measured three ways: the exe's code, the Mac debug-symbol vtable, our own detour VAs). The header mislabels slots 53-57 and 118+ (`GZWinMoveTo` compiles to exe `GZWinOffset`; `SetSize(w,h)` to `SetArea(const cRZRect&)`). RED = our source calls a mislabelled name, or a control moved (GetID/GZPaint must compile to 63/88, GZWinMoveTo must still mismatch). Needs MSVC (vcvars32). | ~5 s |
